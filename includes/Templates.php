 ...
 * 2) parent template, 3) plugin resources. will look in the cp-live/
...
function get_template_hierarchy( $template, $args = [] ) {
	...
	$template_base_paths = apply_filters( 'cp_live_template_paths', ( array ) cp_live()->get_plugin_path() );
	...
	*/
	...
	if ( $overrides_exist ) {
		...
		$file = locate_template( [ 'cp-live/' . $template ], false, false );
		if ( ! $file ) {
			...
			foreach ( array_keys( $template_base_paths ) as $namespace ) {
				if ( ! empty( $namespace ) && ! is_numeric( $namespace ) ) {
					$files[] = 'cp-live' . str_replace( $namespace, '', $template );
				}
			}
			$file = locate_template( $files, false, false );
			if ( $file ) {
				_deprecated_function( sprintf( esc_html__( 'Template overrides should be moved to the correct subdirectory: %s', 'cp-live' ), str_replace( get_stylesheet_directory() . '/cp-live/', '', $file ) ), '3.2', $template );
			}
		} else {
			$file = apply_filters( 'cp_live_template', $file, $template );
		}
	}
	...
	return apply_filters( 'cp_live_template_' . $template, $file );
}
