...
class License {
	...
	public function activate_license() {
		...
		if ( ! isset( $_POST['cp_live_license_activate'], $_POST['cp_live_nonce'] ) ) {
			return;
		}

		// run a quick security check
		if ( ! check_admin_referer( 'cp_live_nonce', 'cp_live_nonce' ) ) {
			return;
		} // get out if we didn't click the Activate button

		// retrieve the license from the database
		$license = trim( get_option( 'cp_live_license_key' ) );
		...
		update_option( 'cp_live_license_status', $license_data->license );
		delete_transient( 'cp_live_license_check' );
	...
	public function deactivate_license() {
		...
		// listen for our activate button to be clicked
		if ( ! isset( $_POST['cp_live_license_deactivate'], $_POST['cp_live_nonce'] ) ) {
			return;
		}
		...
		if ( $license_data->license == 'deactivated' ) {
			delete_option( 'cp_live_license_status' );
			delete_transient( 'cp_live_license_check' );
		}
	...
	public function check_license() {
		...
		if ( ! empty( $_POST['cp_live_nonce'] ) ) {
			return;
		}

		$license = get_option( 'cp_live_license_key' );
		$status  = get_transient( 'cp_live_license_check' );
		...
		if ( $status === false && $license ) {
			...
			update_option( 'cp_live_license_status', $status );

			set_transient( 'cp_live_license_check', $license_data->license, DAY_IN_SECONDS );
